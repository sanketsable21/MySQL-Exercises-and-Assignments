...
INSERT INTO employee (employee_id, employee_name, department_id, faculty_id, position, salary, date_of_joining) 
VALUES
(1, 'Emily Green', 1, 1, 'Professor', 95000.00, '2021-06-01'),
(2, 'Frank Harris', 2, 1, 'Assistant Professor', 85000.00, '2022-08-15'),
(3, 'George White', 3, 2, 'Lecturer', 75000.00, '2023-01-20'),
(4, 'Helen Black', 4, 2, 'Researcher', 78000.00, '2020-03-10'),
(5, 'Ian Red', 5, 3, 'Lab Technician', 60000.00, '2022-11-25');

SELECT employee.employee_id, employee_name, department_name
FROM employee
INNER JOIN department ON employee.department_id = department.department_id;

SELECT employee.employee_id, employee_name, faculty_name
FROM employee
INNER JOIN faculty ON employee.faculty_id = faculty.faculty_id;

SELECT employee.employee_id, employee_name, department_name, faculty_name
FROM employee
INNER JOIN department ON employee.department_id = department.department_id
INNER JOIN faculty ON employee.faculty_id = faculty.faculty_id;
...
SELECT employee.employee_id, employee_name, faculty_name
FROM employee
RIGHT JOIN faculty ON employee.faculty_id = faculty.faculty_id;
...
SELECT employee.employee_id, employee_name, salary.amount
FROM employee
INNER JOIN salary ON employee.faculty_id = salary.faculty_id;

SELECT employee.employee_id, employee_name, students.student_name, department_name
FROM employee
INNER JOIN department ON employee.department_id = department.department_id
INNER JOIN students ON department.department_id = students.department_id;

SELECT employee.employee_id, employee_name, salary.amount, marks.subject_name, marks.marks_obtained
FROM employee
INNER JOIN faculty ON employee.faculty_id = faculty.faculty_id
INNER JOIN salary ON faculty.faculty_id = salary.faculty_id
INNER JOIN marks ON employee.employee_id = marks.student_id;